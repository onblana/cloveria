...
export function ToastStack({ toasts, onDismiss }: ToastStackProps) {
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 top-2 z-40 flex flex-col items-center gap-1 px-2">
      ...
      {/* 감싼 상자가 터치를 흘려보내므로 알림만 다시 받도록 되돌린다 */}
      {toasts.map((toast) => (
        <button
          key={toast.id}
          onClick={() => onDismiss(toast.id)}
          className={`toast-item pointer-events-auto flex min-h-11 max-w-sm items-center whitespace-pre-line rounded-lg px-4 py-2 text-center text-xs text-white ${
            toast.kind === 'milestone' ? 'bg-amber-500/95' : 'bg-neutral-900/85'
          }`}
        >
          {toast.message}
        </button>
