 ...
 * 걷히는 일은 하지 않는다. 문구가 끝나면 덮은 채로 시계 전환에 자리를 넘긴다.
 */
export function RestScreen({ message, onFinish }: RestScreenProps) {
  const [stage, setStage] = useState<'covering' | 'message'>('covering');
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => latest.current.onFinish(), MESSAGE_MS);
